...
export default function CreditUnionsPage({ embedded = false }: { embedded?: boolean }) {
  ...
  useEffect(() => {
    fetch("/api/credit-unions")
      .then((r) => r.json())
      .then((data) => setCreditUnions(data))
  ...
  const handleSave = async (data: Omit<CreditUnion, "id" | "createdAt">) => {
    if (editing) {
      const res = await fetch(`/api/credit-unions/${editing.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(data),
      });
      const updated = await res.json();
      setCreditUnions((prev) => prev.map((cu) => (cu.id === editing.id ? updated : cu)));
    } else {
      const res = await fetch("/api/credit-unions", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(data),
      });
      const created = await res.json();
      setCreditUnions((prev) => [created, ...prev]);
    }
  ...
  const handleDelete = async (id: string) => {
    await fetch(`/api/credit-unions/${id}`, { method: "DELETE" });
